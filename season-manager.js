...
(function () {
  ...
  function injectHistoryNavAndSection() {
    ...
    if (nav && !qs('[data-history-nav]', nav)) {
      ...
      if (rulesBtn) nav.insertBefore(button, rulesBtn);
    ...
    if (!document.getElementById('history')) {
      ...
      if (rulesSection) content.insertBefore(section, rulesSection);
  ...
  function installArchiveGuards(year) {
    ...
    if (!originalApplyLeagueData) {
      originalApplyLeagueData = window.applyLeagueDataFromSheet;
    }
  ...
  function renderHistory() {
    ...
    const playerRows = Object.keys(playerCounts).sort(function (a, b) {
      return playerCounts[b] - playerCounts[a] || a.localeCompare(b);
    });
    ...
    const tableRows = playerRows.map(function (name) {
      return '<tr><td>' + esc(name) + '</td><td>' + playerCounts[name] + '</td></tr>';
    }).join('');

    container.innerHTML =
      '<div class="history-hero">' +
        '<div class="history-kicker">The Cup Lives Here</div>' +
        '<div class="history-title">HGGL Champions</div>' +
        '<div class="history-copy">A permanent record of confirmed Hockey Guys Golf League champions. Additional historical seasons can be added as old league records are verified.</div>' +
      '</div>' +
      '<div class="champion-grid">' + cards + '</div>' +
      '<div class="history-stats">' +
        '<div class="history-stat-card"><b>' + champions.length + '</b><span>Confirmed Seasons</span></div>' +
        '<div class="history-stat-card"><b>' + esc(leaderNames.join(' · ')) + '</b><span>Most Championships (' + mostTitles + ')</span></div>' +
      '</div>' +
  ...
  async function init() {
    try {
      ...
      if (activeSeasonEntry.status === 'archive' && (activeSeasonEntry.data || (activeSeasonEntry.files && activeSeasonEntry.files.length))) {
